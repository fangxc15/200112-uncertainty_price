...
% 这里是个单边市场，感觉就非常僵硬！
Gexp = sdpvar(ng,1);
Dexp_curt = sdpvar(nd,1); %把这个当成一个新发电的东西
beta = sdpvar(ng,1);
...
Cons = [Cons, sum(Gexp) + sum([renewable.Pexp]) - sum([demand.Dexp]) + sum(Dexp_curt) >= 0]; 
Dual.price = length(Cons);

% 对Dexp做限制
Cons = [Cons, Dexp_curt' >= 0];
Cons = [Cons, Dexp_curt' <= [demand.Dexp]];
...
%     Cons = [Cons, aconst.Lmax(i).value' * x + Qconst' * w.Lmax(i).varible + anx2 <= bconst.Lmax(i).value];
    Cons = [Cons, aconst.Lmax(i).value' * x + GSDFD_ld(i,:) * Dexp_curt + Qconst' * w.Lmax(i).varible + anx2 <= bconst.Lmax(i).value];
    Dual.Lmax(i) = length(Cons);
%     Cons = [Cons, aconst.Lmin(i).value' * x + Qconst' * w.Lmin(i).varible + anx2 <= bconst.Lmin(i).value];
    Cons = [Cons, aconst.Lmin(i).value' * x - GSDFD_ld(i,:) * Dexp_curt + Qconst' * w.Lmin(i).varible + anx2 <= bconst.Lmin(i).value];
...
%% 目标函数
obj = [generator.cost] * Gexp - [demand.Duti] * ([demand.Dexp]' - Dexp_curt);
