function success = F_plot_interval(N,X1,X1_max,X1_min,X2,X2_max,X2_min,Linemax,xlabelname,ylabelname,titlename,yrange,marker1,marker2,legendname, savename)
    % 这里还要画出linemax
    % X3,X4 是可选的。
    if isempty(savename)
        savename = titlename;
    ...
    if ~isempty(Linemax)
        hold on
        plot(1:N,Linemax,['--','k'],1:N,-Linemax,['--','k']);
    end
