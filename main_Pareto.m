% 该函数用来求Pareto Frontier
% 这里要求的是激励相容和社会福利最大化的权衡。
% 我需要研究一下某些False_agent(1:30)，去查看他的参数变化时候的出清量变化，进而计算他的福利。
% 在不同的kIC之下有多少收支不平衡量。
% 由于需要严格满足收支平衡，只能调整spread
% 所以设计了Spread_array
...
elseif nnode == 5
    ...
    falseagent = 3:5;
    Setting.falseagent = falseagent;
    spread_array = 0:0.5:6; %只对火电成本做增加，防止出现畸变
end
%% 修正发电机容量
if nnode == 5
    mpc.gen(1,9) = 110;
...
outall.RelaxIC = outall.RelaxIC(qualify_set);
outall.SW = outall.SW(qualify_set);
outall.RelaxSW = outall.RelaxSW(qualify_set);
outall.spread = outall.spread(qualify_set);



mkdir(['result_Pareto',version_suffix])
save(['result_Pareto',version_suffix,'//',Filename])

Picture_root_folder = ['Picture', version_suffix];  
mkdir(Picture_root_folder);
%     Picture_folder = Picture_root_folder;
Picture_folder = [Picture_root_folder,'/',Filename];
mkdir(Picture_folder);

    
F_plot_Pareto(1, outall,Picture_folder)


%% 如何分析谎报
% 这个相当于在spread = 0的时候，去查看真实申报下的利润
for pno = 0:Setting.NumP
